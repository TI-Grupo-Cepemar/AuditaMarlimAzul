...
'dynamic' AS component,
sqlpage.run_sql('..\view_configs\componentes_dinamicos\modal.sql', json_object('id_modal':'modal_editar_'||g.id_gestor,'titulo_modal':'Editar gestor','url_iframe':'\entidades\gerentes\formularios\editar?id_gerente='||g.id_gestor)) AS properties
FROM gestores g;

SELECT -- POPUP QUE APARECERÁ QUANDO O REQUISITANTE CLICAR EM REMOVER UM GERENTE
'dynamic' AS component,
sqlpage.run_sql('..\view_configs\componentes_dinamicos\modal.sql', json_object('id_modal':'modal_remover_'||g.id_gestor,'titulo_modal':'Remover gestor','url_iframe':'\entidades\gerentes\formularios\remover?id_gerente='||g.id_gestor)) AS properties
FROM gestores g;
...
SELECT 
ROW_NUMBER() OVER (ORDER BY g.id_gestor) AS "ID",
g.nome AS "Nome", 
