...
sqlpage.run_sql('..\view_configs\controle_de_acesso.sql', json_object('funcao','42')) AS properties; -- Permissão 42) Visualizar grupos de responsabilidade
...
SELECT -- POPUP QUE APARECERÁ QUANDO O REQUISITANTE CLICAR PARA CADASTRAR UMA CAUSA
'modal' AS component,
'cadastroCausa' AS id,
'Cadastrar causa' AS title,
'\entidades\causas\formularios\' AS embed;

SELECT -- POPUP QUE APARECERÁ QUANDO O REQUISITANTE CLICAR EM EDITAR UMA CAUSA
'dynamic' AS component,
sqlpage.run_sql('..\view_configs\componentes_dinamicos\modal.sql', json_object('id_modal':'modal_editar_'||c.id,'titulo_modal':'Editar causa','url_iframe':'\entidades\causas\formularios\editar?id_causa='||c.id)) AS properties
FROM causas c;

SELECT -- POPUP QUE APARECERÁ QUANDO O REQUISITANTE CLICAR EM REMOVER UMA CAUSA
'dynamic' AS component,
sqlpage.run_sql('..\view_configs\componentes_dinamicos\modal.sql', json_object('id_modal':'modal_remover_'||c.id,'titulo_modal':'Remover causa','url_iframe':'\entidades\causas\formularios\remover?id_causa='||c.id)) AS properties
FROM causas c;
