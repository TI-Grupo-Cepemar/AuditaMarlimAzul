...
function SelectRelations() {
  ...
  function updateFiltering(select, restart = true) {
    ...
    document
      .querySelectorAll(`[data-sf-parent*="${parentId}"]`)
      .forEach((childSelect) => {
            ...
            if (relationData) {
              const displayOption = relationData.split("&").every((pair) => {
                ...
                if (el_parent.type == "checkbox")
                  ...
                  return (
                    ids.includes(el_parent.value) ||
                    (el_parent.options[el_parent.selectedIndex] != undefined &&
                      ids.includes(
                        el_parent.options[el_parent.selectedIndex].dataset.alt
                      ))
                  );
              });
  ...
  function resetParentSelects() {
    document.querySelectorAll(`[data-sf-parent]`).forEach((childSelect) => {
      if (childSelect.tagName == "SELECT") {
        ...
        if (
          childSelect.selectedIndex == null ||
